...
Один сервис должен быть одним бинарником со стандартными метриками и всем остальным. У нас были сервисы, написанные на Ruby — «один сервис — один бинарник» им, соответственно, не подходил. Сервисы, которые оставались на Ruby, мы тоже переписали на Go, чтобы можно было их собирать.
...
Если у нас ничего лишнего нет, то ломаться нечему, и ресурсы тратить тоже особо некому. systemd и так работает. Да, я слышал жалобы на systemd, что он якобы ест какое-то нереальное количество ресурсов, но в нашей практике его накладные расходы практически незаметны. И это, соответственно, надёжно: если systemd не работает — у нас вообще ничего не работает, так что это базовая штука, за которой следят все.
...
Эксплуатация занимается базой: когда у нас появляется сервер и к нему подвели сеть, на него накатывается базовая роль. Дальше сервер добавляется в inventory, отключается root-доступ, настраиваются доступы, и ставится мониторинг/логирование: например, node exporter и агент для сбора логов. Мы собираем вообще все **логи** в системе — не только наши сервисы, а вообще всё, что есть. Нужно это редко, но метко: можно одномоментно посмотреть, что происходило на хосте.
...
- **Нельзя локально поднять “всю систему одной командой”** — новичкам это иногда было бы удобно. Но по большому счёту и не нужно: сервисов много, а чтобы работать над конкретным сервисом, не надо поднимать вообще всё. Обычно достаточно базы данных, nginx и самого сервиса
