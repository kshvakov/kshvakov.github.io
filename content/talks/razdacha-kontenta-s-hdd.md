...
В этом докладе я рассказываю о нашем опыте построения CDN для раздачи видеоконтента на обычных жёстких дисках. Когда мы столкнулись с проблемой миллиардов мелких файлов и достигли предела производительности на уровне 4 гигабит на сервер, пришлось переосмыслить подход к хранению и раздаче данных.
...
У нас есть простая эвристика: «крупные» объекты сразу пишем на HDD (в докладе звучит порог порядка ~225 МБ), а мелочь — сначала на SSD.
...
В результате мы существенно (в докладе звучит «примерно на порядок») снизили random I/O на дисках.
...
Размер кэша порядка десятков гигабайт (в докладе — около 30 ГБ). Политика простая — LRU: она хорошо закрывает «горячий» контент.
...
Мы стали смотреть, чем можно терминировать TLS. В докладе упоминается **Hitch** (проект Varnish): он, по сути, делает одну вещь — принимает TLS, расшифровывает и передаёт дальше (в TCP или Unix socket).
...
Отдельная надежда — **kTLS**: ядро умеет часть работы по TLS брать на себя. В докладе упоминается, что в Linux это появилось относительно недавно (например, в районе 5.3), и это потенциально может заметно ускорить терминацию[^ktls-details].
...
Когда дошли до десятков миллионов файлов на сервер (в докладе — порядка 50–80 млн), стало видно, что значимая часть CPU уходит на работу GC.
...
Ещё один эффект: из‑за накладных расходов модели памяти в Go мы в среднем тратим заметно больше RAM, чем хотелось бы. В результате страдает **page cache** — а в Linux это очень полезная штука для ускорения чтения. В докладе это связывается и с тем, что у нас нет `O_DIRECT`.
...
Из того, что в докладе упоминается «на почитать»:
...
**Ответ**: Да, размер фиксирован: партиции создаются при инициализации диска. Метаданные — фиксированного размера (в докладе звучит порядок сотен мегабайт, например ~200 МБ), потому что записи там фиксированной структуры.

Сами партиции получаются из разбиения диска на равные части (в докладе — на 50; отдельно отмечается, что это решение потом захотелось пересмотреть). При объёмах уровня 100+ ТБ «крупных» контейнеров всё равно получается очень много.
