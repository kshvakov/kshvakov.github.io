...
Эволюция нашей архитектуры: от первоначальной схемы с Vertica и Citus до перехода на ClickHouse, проблемы с материализованными представлениями, оптимизацию индексов и финальное решение с использованием массивов и структур данных.
...
- **Резкие всплески трафика** при подключении новых площадок (publishers) (легко поднимается на 5–10 тысяч запросов в секунду)
...
Число 8192 — это гранулярность индекса по умолчанию в ClickHouse[^granularity-default]. ClickHouse индексирует не каждую строку, а каждые 8192 строки (по умолчанию) — хранит min/max значения для гранулы.
...
- [Документация ClickHouse](https://clickhouse.com/docs) — официальная документация с примерами, best practices и описанием всех возможностей системы
- [ClickHouse для начинающих](https://clickhouse.com/docs/en/guides/getting-started) — пошаговое руководство по установке и первым запросам
- [Оптимизация запросов в ClickHouse](https://clickhouse.com/docs/en/guides/improving-query-performance) — практические советы по ускорению запросов, работе с индексами и партициями
- [Массивы и структуры данных в ClickHouse](https://clickhouse.com/docs/en/sql-reference/data-types/array) — подробное описание работы с массивами и Map, которые помогли нам сократить количество таблиц
- [Материализованные представления](https://clickhouse.com/docs/en/sql-reference/statements/create/view#materialized-view) — документация по materialized views и их ограничениям при высокой нагрузке записи
- [MergeTree движки](https://clickhouse.com/docs/en/engines/table-engines/mergetree-family) — описание различных движков MergeTree и когда какой использовать
- [Гранулярность индексов](https://clickhouse.com/docs/en/guides/improving-query-performance/sparse-primary-indexes) — как работает индексация в ClickHouse и почему гранулярность критична для производительности
- [Оптимизация памяти в ClickHouse](https://clickhouse.com/docs/en/operations/memory-limits) — настройки лимитов памяти и работа с большими запросами
- [ClickHouse в продакшене: опыт Yandex](https://habr.com/ru/company/yandex/blog/303290/) — практический опыт масштабирования ClickHouse на больших объёмах данных (на русском)
- [Производительность ClickHouse: внутреннее устройство](https://clickhouse.com/docs/en/development/architecture) — архитектура системы и как она влияет на производительность запросов

## Сноски

[^granularity-default]: Гранулярность индекса по умолчанию в ClickHouse — 8192 строки. Это значение можно изменить через параметр `index_granularity` при создании таблицы. Подробнее см. [документацию ClickHouse](https://clickhouse.com/docs/en/engines/table-engines/mergetree-family/mergetree#mergetree-data-storage).
