...
В этой статье — практическая сторона задачи: почему TLS ломает **zero‑copy** (передачу без копирования), зачем нужен kTLS, что именно меняется в пути данных, где появляется реальный выигрыш и какие грабли встречаются в продакшене (включая неожиданный рост хендшейков). В конце есть отдельный раздел **«Как измерять»** — что смотреть в `perf` и `pprof`, чтобы не оптимизировать вслепую.

Важно: «100 Gbps» в названии — не «предел технологии», а ориентир под **конкретное железо и сетевую конфигурацию**. На другой платформе (CPU, память, ядро, сетевые карты) тот же подход может дать заметно больше — и наоборот.
...
- **kTLS переносит слой TLS‑записей (record layer) в ядро**: вы пишете в сокет «обычные» данные, а ядро само формирует TLS‑записи и шифрует. Это возвращает zero‑copy (в первую очередь на отправке).
...
- **Важно**: kTLS почти не помогает, если вы *генерируете* контент в userspace. Максимальный эффект — когда вы **отдаёте «как есть»** (файлы/чанки) и можете включить zero‑copy.
...
В этом тексте я почти не касаюсь криптографии (это отдельная большая тема). Нам важна инженерная механика:
...
Дальше я буду использовать два термина:
...
### Мини‑схема: что именно меняется, когда вы «включаете TLS»
...
Технически всё выглядит просто:

1. Клиент и сервер устанавливают TCP‑соединение.
2. Проходит TLS‑хендшейк: выбираются версии/шифры, обмениваются параметрами, договариваются о ключах.
3. Дальше по этому соединению идут зашифрованные данные.
...
И если TLS реализован в userspace, вы почти неизбежно приходите к схеме:

1. прочитали данные (в userspace);
2. зашифровали (в userspace);
3. записали в сокет.

То есть zero‑copy исчезает — и начинаются накладные расходы.

### Мини‑схема: почему `io.Copy` — не гарантия zero‑copy в Go
...
Поэтому, когда вы оптимизируете «раздачу файла», полезно проверять не только CPU‑профили, но и то, каким путём реально пошло копирование (ниже, в разделе «Как измерять», есть подсказки, как это увидеть).

## «Дешёвые клиенты» и шифры: сначала убедитесь, что проблема у вас есть

Перед тем как «чинить TLS», полезно сделать первую вещь: **померить**.

В докладе упоминается, что мы добавили метрики по используемым шифрам (cipher suites): профиль сильно зависит от клиентской базы. На практике часто оказывается, что часть клиентов выбирает шифры, которые на сервере работают дороже. Иногда это ещё и ломает аппаратное ускорение — и становится «вдруг дорого».

Практический смысл:

- вы видите, что реально используется в проде;
- понимаете, какой процент трафика потенциально «дорогой»;
- и уже потом решаете, что делать: менять политику шифров, менять версию TLS, включать/выключать конкретные варианты и т. п.
...
Идея в том, что **хендшейк** вы всё равно делаете обычной TLS‑библиотекой (в userspace). Но как только ключи получены, вы **передаёте их ядру**, включаете режим kTLS для сокета — и дальше работаете с соединением «как с обычным»:

- пишете в сокет **не зашифрованные** данные;
- ядро шифрует их само.

Ключевое: если ядро умеет шифровать на отправке, оно может снова использовать zero‑copy‑механизмы и сократить копирования/аллокации.

### История идеи: Netflix/Facebook и внезапно Oracle

В докладе упоминается забавный исторический момент: «новую» технологию обсуждали Netflix (FreeBSD) и Facebook (Linux), но если копнуть глубже, похожая идея была ещё раньше — у Oracle в Solaris.

То есть концепция «перенести TLS ближе к ядру/сетевому стеку ради производительности» — не «модная прихоть», а закономерное следствие желания убрать копирования.

### Дополнительный бонус: offload на сетевую карту

В некоторых сценариях TLS можно частично оффлоадить на NIC (TLS offload). В докладе отдельно упоминается, что на практике это сильно зависит от доступности железа и окружения (в РФ это может упираться даже в логистику/криптографию/сертификацию оборудования).
...
2. **Нужно поменять путь записи**: после включения kTLS в сокет нельзя продолжать писать туда уже зашифрованные TLS‑записи — нужно писать «чистые» данные.

Поэтому в реальности (в докладе) был сделан небольшой патч/форк TLS‑части, который:
...
### Типичная ошибка: включили kTLS и… всё сломали

Одна из центральных мыслей доклада: после включения kTLS вы должны **перестать писать TLS‑записи** в сокет. Нужно писать обычные данные, а record layer и шифрование становятся задачей ядра.

Если это перепутать, получится «вроде всё включили, но не работает» — просто потому, что теперь вы шифруете два раза или вообще пишете не тем форматом.

## Где выигрываем: `sendfile`, `splice` и «файлы как есть»

Отдельная важная деталь из Go‑мира: чтобы приложение действительно использовало zero‑copy, недостаточно «просто `io.Copy`».
...
В докладе звучат ориентиры уровня:
...
Отдельно подчеркну: потолок здесь задаёт не «Go» и не «kTLS», а **конкретная платформа** (полоса памяти, CPU, сеть, драйверы, версия ядра, баланс IRQ/softirq, конфигурация NIC). На другом железе вы упрётесь в другие лимиты и можете увидеть как меньшие, так и значительно большие числа.

Важно правильно прочитать эти цифры:
...
В докладе упоминаются два практических шага:

### 1) Уход от RSA‑сертификатов к ECDSA

Если у вас в цепочке всё ещё RSA‑сертификаты, то для части CPU это может быть заметно дороже, чем ECDSA. В докладе это звучит максимально прагматично: «у вас RSA — просто замените сертификаты».

Там же приводится порядок цифр из простого бенчмарка: на RSA хендшейк условно «в несколько раз» дороже, чем на эллиптических кривых (естественно, числа зависят от CPU и окружения, важен именно порядок).

### 2) Возобновление сессии (session resumption) и общий ключ тикетов
...
В докладе это решалось просто: задавали общий ключ (в рамках CDN‑контента это допустимый компромисс, потому что речь не про «секретные данные», а про ускорение раздачи публичного контента).

### Неочевидная причина всплеска хендшейков: HTTP 400+ и «умные» браузеры

В докладе есть очень практичная деталь: если вы отдаёте видео и легитимно используете **коды ответа 400+** (например, для манифестов/управления), браузеры могут вести себя «слишком умно»: закрывать соединение и открывать новое.
...
#### Мини‑схема: как «легитимные 4xx» превращаются в «шторм хендшейков»
...
Ниже — практический минимум, который помогает понять, *где вы теряете скорость*: в криптографии, в копированиях, в GC, в системных вызовах, в ядре или в сетевом стеке.
...
1) Включите `net/http/pprof` (если ещё не включено) и снимайте профили в момент пика.

2) Что смотреть в CPU‑профиле:
...
3) Что смотреть в профилях alloc/heap:
...
#### 1) `perf top`: посмотреть «кто в топе» прямо сейчас
...
#### 2) `perf record`/`perf report`: снять профиль на интервал и спокойно разобрать
...
#### 3) `perf stat`: быстро проверить «копии/кэш/ветвления» (если доступно)

```bash
sudo perf stat -p <PID> -- sleep 10
```

Смысл: увидеть, не упираетесь ли вы в кэш‑промахи/ветвления/инструкции на байт полезной работы. На «протоке» это часто важнее «процента CPU».
...
- **kTLS не ускорит генерацию контента**: если вы «рисуете» данные в памяти, то вы всё равно делаете работу в userspace, и выигрыш будет небольшим или его не будет.
...
## Мини‑чеклист: если вы захотите повторить
...
- **И только потом — kTLS**:
  ...
  - убедиться, что после включения kTLS вы пишете *plaintext* данные;
  - прогонять на ядрах/драйверах, которые вы реально используете в проде.

## Итоги

Главная мысль доклада простая: на очень больших скоростях у вас «дорогой» не TLS как концепция, а **всё, что заставляет данные лишний раз проходить через память приложения**.
...
А если вы генерируете контент в userspace, то kTLS — это не серебряная пуля: сначала нужно оптимизировать сам путь данных и аллокации.
