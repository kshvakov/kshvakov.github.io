...
В идеале приложение делает минимум: формирует и записывает HTTP‑заголовки в сокет, а затем говорит ядру: «вот файл, вот сокет».
...
Для нас это критично, потому что раздача — это не «несколько килобайт HTML», а **мегабайты и гигабайты**. (Подробнее о практическом применении zero‑copy в CDN — см. [«Раздача контента с HDD»]({{< relref "talks/razdacha-kontenta-s-hdd.md" >}}).)
...
История kTLS на самом деле началась не с Linux и FreeBSD. Ещё в Oracle Solaris существовал **KSSL** — kernel-side proxy для SSL/TLS, который делает обработку SSL-трафика в ядре (идея та же: сдвинуть record layer ближе к сетевому стеку ради производительности) — см. описание в документации Oracle Solaris ([kssl(5)](https://docs.oracle.com/cd/E23823_01/html/816-5175/kssl-5.html)).

В FreeBSD **Kernel TLS** впервые появился в **FreeBSD 13.0** ([ktls(4)](https://man.freebsd.org/cgi/man.cgi?query=ktls&sektion=4)); релиз **FreeBSD 13.0-RELEASE** датирован **April 13, 2021** ([announcement](https://www.freebsd.org/releases/13.0R/announce/)). Однако стоит отметить, что **Netflix использовал kTLS в FreeBSD значительно раньше** — компания начала внедрять эту технологию ещё с 2016 года для оптимизации производительности при раздаче видео-контента, задолго до того, как kTLS попал в официальный релиз FreeBSD.

В Linux kTLS попал в mainline **Jun 15, 2017**: коммит **"tls: kernel TLS support"** добавляет TLS как ULP поверх TCP, при этом ключи передаются в ядро через `setsockopt` после завершения handshake, а в ядре выполняется только симметричное шифрование ([commit 3c4d755](https://github.com/torvalds/linux/commit/3c4d7559159bfe1e3b94df3a657b2cda3a34e218)). Пользовательский интерфейс и модель работы (TLS ULP, раздельный TX/RX, пример с `sendfile`) описаны в официальной документации ядра Linux ([Kernel TLS docs](https://docs.kernel.org/networking/tls.html)).
...
**Порядок цифр** из простого бенчмарка: на RSA хендшейк условно «в несколько раз» дороже, чем на эллиптических кривых (естественно, числа зависят от CPU и окружения, важен именно порядок). Подробнее о проблеме производительности RSA handshake в Go см. [issue #20058](https://github.com/golang/go/issues/20058).
...
- [crypto/tls: slow server-side handshake performance for RSA certificates](https://github.com/golang/go/issues/20058) — известная проблема производительности RSA handshake в Go: детальный анализ узких мест и сравнение с OpenSSL
...
[^cdn-background]: Подробнее о том, как мы строили собственную CDN и почему это было необходимо, см. [«Раздача контента с HDD»]({{< relref "talks/razdacha-kontenta-s-hdd.md" >}}).
