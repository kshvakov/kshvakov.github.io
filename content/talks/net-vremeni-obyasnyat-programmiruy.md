...
Запросы обычно приходят очень маленькие, заголовки и всё, больше ничего нет. Мы получаем response и просто делаем response, который есть, у него есть метод Write, и мы пишем в соединение, и в принципе всё здорово.
...
Одна проблема: стандартный не отправляет, условно то, копирует в соединение данные из-под себя. Он копирует их, копирует сначала в память, то есть из user space мы поднимаем, обычный подход. Такого можно избежать.
