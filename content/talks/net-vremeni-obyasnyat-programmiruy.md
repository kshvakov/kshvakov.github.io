...
Эта статья подготовлена по мотивам доклада «Нет времени объяснять, программируй!» про опыт разработки собственных решений в Kinescope: когда стоит писать свой софт, какие проблемы это решает и какие сложности возникают.
...
Если вернуться назад, то был 2020 год. Все ждали Олимпиаду в Токио, кто-то сходил с ума, а мы начинали писать Kinescope. В этот год все сидели по домам, и мы писали.
...
И задачи стали расти. Бизнес растёт, поддержка нужна. Вроде бы всё хорошо, но времени нет — что-то пилим, задачи, клиенты, клиенты.
...
Хорошие примеры: Go появился под реальные задачи, писался людьми, которые сталкивались с проблемами и решали их. ClickHouse — та же история. MySQL и InnoDB — отличные штуки, их любят все, даже те, кто пользуется PostgreSQL, потому что это произведение искусства. Советую почитать код, если интересно занимаетесь базами данных или работой с дисками — хорошая штука для погружения.
...
**Должен быть список ограничений.** Когда вы разрабатываете технологию, особенно не в стол, а под бизнес, у вас должно быть понимание, что эта штука может не решить, или что можно выкинуть. Ограничения — это хорошо, и мы ими сильно пользуемся в наших решениях, потому что выкидываем очень много того, что нам не надо, и вообще не реализуем.
...
Для видеоконтента, которым мы занимаемся, у нас так и было: поставили nginx, поставили модули, всё в принципе работало. Но в какой-то момент нам стало этого не хватать.
...
SSD — почему так: потому что это дешевле. Если в нас засунуть SSD, мы будем работать лучше. Если не засунуть SSD, то в принципе тоже всё работает хорошо. У нас на рынке один из самых низких latency-расходов.
...
Если раньше мы покупали супермикру, и оно было всё одинаковое, его одинаково настраивали, вливали. Тут у нас появляются старые, разные серверы, разные жёсткие диски. Кстати тоже хорошо, что это на софте, поэтому можно втыкать разное, разного вендора, вообще разные серверы могут быть.
...
А на сервере полно памяти свободной, всё неплохо по CPU, в принципе всё неплохо, хорошо, но ползёт I/O, и реально мы видим, что скорость I/O конкретно на этом сервере плавает. Мы тут же коррелируем с трафиком, смотрим, что так быть не должно.
...
Почему мы не использовали HTTP-сервер? Он не имел возможностей для наших задач. Количество RPS у нас не такое большое — сервер может пролететь пару десятков тысяч, это в принципе наверное выдержит стандартный.
...
У нас есть прокси. Что делает прокси? Он принимает трафик, разбирает HTTP-запрос, отправляет его по TCP, берёт nginx и отправляет его выше. Казалось бы просто.
...
Там давно-давно в Go был io.Reader, который добавил методы. Это либо Seek, если мы работаем с файлами, либо WriteTo, если мы работаем с сетью. Но внутри её копия можно поковыряться, он там проверяет интерфейс, если интерфейс совпал, то типа всё О'кей, отправляет.
...
Потому что мы грубо говоря что делаем при zero-copy: мы говорим, вот у тебя есть один дескриптор, вот у тебя есть другой дескриптор, ты там сама разбирается операционная система, то есть мы так делаем это и для файлов, и мы так делаем для коннектов. Поэтому здесь мы не просачиваемся с точки зрения Go, то есть здесь мы не ловим эффекты, когда мы с garbage collector боремся, ещё с какой-то историей, здесь о нас вообще никак не относится, то есть отдал и забыл.
...
Мы как самые умные, когда стартовали, напомню, что у нас там 250 плюс 1000 объектов памяти или миллионов, короче много. Мы написали дерево, причём было чисто потренироваться, грубо говоря, разбавить память, написали дерево, чтобы хранить индекс в памяти, всё хорошо, начало заливаться, прям работает, работает достаточно быстро.
...
В итоге просто взяли, попробовать, блин, хватит бороться, давайте просто возьмём map и запишем ключ-значение, всё, и оно блин взяло и поехало, бывает.

Нужны sync.Pool опять же, тоже самое: мы выделяем память какую-то, лучше её не отдавать сразу, если у нас есть какой-то объект, либо там байты, ещё что-то, это всё в sync.Pool хранится, чтобы каждый раз как collector не пытался это дело вычищать, мы это переиспользуем постоянно.

Поэтому у нас каких таких больших проблем с памятью нет.

## TLS и производительность

С какого-то момента весь трафик передаётся шифрованный, и соответственно мы написали своё решение. Хотелось бы подключить сертификат и поехать — мы всё равно бы написали свой веб-сервер, написали там работу с этой штукой, и почему бы нет, попробовали, всё работает с коробки, прям хорошо, отличный интерфейс, касаемо именно стандартной библиотеки интерфейсов.
...
Сильно упростили. У нас нет никакого persistent cache, то есть ты его ребутнуть, есть файлик на диске, обычные, кольцевой буфер пишем с файла от начала до конца, когда заканчивается, начинаем писать с начала и затираем то, что есть. Вот тебе вытеснение, всё просто, нет никакого шардирования, очередей на запись, вообще тупой, всё просто.
...
Если ценность решения есть, то можно его делать и 2 года. А если что-то не работает, то это на самом деле не означает, что проблема конкретно в вас, но может означать такое тоже бывает, нужно профилировать, это всегда есть, можно насколько угодно иметь огромное количество опыта и накосячить прямо на простейшем, и это в принципе нормально.
