...
Подробнее о практической настройке мониторинга, типовых проблемах и их решении см. в статье [«Эксплуатация без K8s»]({{< relref "talks/ekspluatatsiya-bez-k8s.md" >}}).
...
Подробнее о формате `spec.yml` и практических примерах см. в статье [«Эксплуатация без K8s»]({{< relref "talks/ekspluatatsiya-bez-k8s.md" >}}).
...
- [Наблюдаемость без Kubernetes](https://grafana.com/docs/loki/latest/) — Loki для централизованного сбора логов и Grafana для визуализации метрик
- [Prometheus: мониторинг и метрики](https://prometheus.io/docs/introduction/overview/) — система мониторинга, которая работает независимо от оркестраторов
- [BGP и anycast для отказоустойчивости](https://www.cloudflare.com/learning/security/glossary/what-is-anycast/) — как использовать BGP/anycast для маршрутизации трафика
- [Простота vs сложность в инфраструктуре](https://blog.cloudflare.com/cloudflare-outage/) — примеры того, как излишняя сложность может привести к проблемам
- [Kubernetes: когда он оправдан](https://kubernetes.io/docs/concepts/overview/what-is-kubernetes/) — официальная документация Kubernetes (полезно для понимания, когда он действительно нужен)
- [Go: простота эксплуатации](https://go.dev/doc/effective_go) — как Go упрощает доставку и эксплуатацию приложений
